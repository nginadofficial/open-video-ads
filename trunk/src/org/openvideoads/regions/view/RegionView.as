	...
	public class RegionView extends Sprite {
		...
		public function RegionView(controller:RegionController, regionConfig:RegionViewConfig, displayProperties:DisplayProperties, showCloseButton:Boolean=true) {
			...
			if(_config.showCloseButton) {
				_closeButton = new CrossCloseButton();
		...
		private function arrangeText():void {
			if(_text) {
				...
				if (padding.indexOf(" ") > 0) {
					...
					_text["height"] = height - NumberUtils.toPixels(pads[0]) - NumberUtils.toPixels(pads[2]);
					_text["width"] = width - NumberUtils.toPixels(pads[1]) - NumberUtils.toPixels(pads[3]);
				}
				else {
					var paddingInPixles:int = NumberUtils.toPixels(padding);
					_text["y"] = paddingInPixles[0];
					_text["x"] = paddingInPixles[3];
					_text["height"] = height - paddingInPixles[0] - paddingInPixles[2];
					_text["width"] = width - paddingInPixles[1] - paddingInPixles[3];
				}
			}
		}
