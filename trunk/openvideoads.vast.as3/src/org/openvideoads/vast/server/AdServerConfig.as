	...
	public class AdServerConfig extends Debuggable {
		...
		public function initialise(config:Object):void {
			if(config != null) {
				...
				if(config.adParameters != null) _selectionCriteria = config.adParameters;
				if(config.allowAdRepetition != undefined) allowAdRepetition = config.allowAdRepetition;
				if(config.type != undefined) _type = config.type;
			}
		}
		...
		public function set adParameters(adParameters:Array):void {
			_selectionCriteria = adParameters;	
		}

		public function get adParameters():Array {
			return _selectionCriteria;	
		}
