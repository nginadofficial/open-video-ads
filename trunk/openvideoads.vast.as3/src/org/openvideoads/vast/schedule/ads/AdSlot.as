...
package org.openvideoads.vast.schedule.ads {
	...
	public class AdSlot extends Stream {
		...
		public function AdSlot(parent:StreamSequence,
		                       ...
		                       streamType:String="mp4", 
		                       deliveryType:String="streaming", 
		                       bitrate:String="any", 
		                       playOnce:Boolean=false,
		                       metaData:Boolean=true) {
			super(parent, vastController, key, id, null, startTime, duration, originalDuration, false, null, streamType, deliveryType, bitrate, playOnce, metaData);
            _owner = owner;
		...
		public override function declareTrackingPoints(currentTimeInSeconds:int=0):void {
			if(_trackingPointsSet == false) {
				...
				if(hasCompanionAds()) { 
					addCompanionAdTrackingPoints(key, currentTimeInSeconds);
				}			
		...
		protected function addCompanionAdTrackingPoints(adSlotIndex:int, currentTimeInSeconds:int):void {
			...
			setTrackingPoint(new TrackingPoint((startPoint * 1000) + EVENT_DELAY, "CS", new String(adSlotIndex))); 

			if(duration > 0) {
				setTrackingPoint(new TrackingPoint((((startPoint + duration) * 1000) - _vastController.endStreamSafetyMargin) - EVENT_DELAY, "CE", new String(adSlotIndex)));
		...
		public function addNonLinearAdTrackingPoints(adSlotIndex:int, resetStartTimeToZeroEachStream:Boolean=false, checkCompanionAds:Boolean=false, overrideSetFlag:Boolean=false):void {
			...
			if(checkCompanionAds && hasCompanionAds()) {
				addCompanionAdTrackingPoints(adSlotIndex, startPoint); 
			}
			...
			if(hasLinearAd()) {
				...
				if(hasCompanionAds()) { 
					addCompanionAdTrackingPoints(key, 0);
				}							
			}
		}
