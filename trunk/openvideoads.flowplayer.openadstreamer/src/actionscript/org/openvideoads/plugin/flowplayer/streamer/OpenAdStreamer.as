...
package org.openvideoads.plugin.flowplayer.streamer {
	...
	public class OpenAdStreamer extends AbstractSprite implements Plugin {
        ...
        public static var OAS_VERSION:String = "v0.2.4";
		...
		protected function checkProviderAvailability():void {
			if(_vastController.needsProvider("rtmp")) {
				ensureProviderPluginLoaded(
					"rtmp", 
					"rtmp",
					function onProviderLoaded(pluginModel:PluginModel):void {
						doLog("*** RTMP provider plugin loaded");
						ensureProviderPluginLoaded(
							"rtmpInstream",
							"rtmp",
							function onInstreamProviderLoaded(pluginMode:PluginModel):void {
								doLog("*** Instream RTMP provider plugin loaded");
							}
						);
					}
				);
		...
		protected function ensureProviderPluginLoaded(providerName:String, providerPluginName:String, callback:Function):void {
			if(_player.pluginRegistry.getPlugin(providerName) == null) {
			   ...
			   if(providerUrl != null) {
					...
					_player.loadPlugin(providerName, providerUrl, callback);	   	
			   }
			   else doLog("Could not load provider " + providerName + " - no load information found", Debuggable.DEBUG_FATAL);
			}
		}
		...
		protected function onStreamSchedule(event:StreamSchedulingEvent):void {				
			...
			clip.autoPlay = (event.scheduleIndex == 0) ? _vastController.autoPlay() : true; //event.stream.autoPlay;
		...
		private function onPauseEvent(playlistEvent:ClipEvent):void {
        	if(_vastController != null) _vastController.onPlayerPause(_player.playlist.currentIndex);
		}

		private function onResumeEvent(playlistEvent:ClipEvent):void {
        	if(_vastController != null) _vastController.onPlayerResume(_player.playlist.currentIndex);
		}
